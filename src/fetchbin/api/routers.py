...
def view_all_outputs(request: Request, session: Session = Depends(get_db_session)):
...
def view_raw_output(public_id: str, session: Session = Depends(get_db_session)):
...
def view_output(request: Request, public_id: str, session: Session = Depends(get_db_session)):
    statement = select(database.FetchOutput).where(database.FetchOutput.public_id == public_id)
    db_output = session.exec(statement).first()
...
def delete_page(request: Request, delete_token: str, session: Session = Depends(get_db_session)):
...
def delete_output(request: Request, delete_token: str, session: Session = Depends(get_db_session)):
    statement = select(database.FetchOutput).where(database.FetchOutput.delete_token == delete_token)
    db_output = session.exec(statement).first()
...
def share_output(request: models.ShareRequest, session: Session = Depends(get_db_session)):
...
def upvote_output(public_id: str, request: Request, session: Session = Depends(get_db_session)):
...
def downvote_output(public_id: str, request: Request, session: Session = Depends(get_db_session)):
    share_statement = select(database.FetchOutput).where(database.FetchOutput.public_id == public_id)
    db_share = session.exec(share_statement).first()
